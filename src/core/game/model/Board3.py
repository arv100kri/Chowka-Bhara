...
The heuristic function is H4 for the IntelligentAgent
...
The heuristic function is H1 for the Naive Agent
w1*f1+w2*f2/(w5*f5)
f1: Hitting opponent pawn
f2: Moving to safe square
f5: Distance from goal square
...
class Board(object):
    ...
    def choosePawnIntelligentVsIntelligent(self):
    ...
    def naiveEvaluationFunction(self, pawnToBeEvaluated, diceValue):
        ...
        print "Naive Level 1 evaluation for pawn", pawnToBeEvaluated.getName()[1],"of player: ", self.__currentPlayer.getPlayerName()," when the dice value is: ", diceValue,": ", str(evaluatedValue/float(distanceToGoalSquare+distanceToSafeSquare))
        return evaluatedValue/float(distanceToGoalSquare+distanceToSafeSquare)    

    #The pawn to be evaluated will only be a pawn which can be moved
    def naiveEvaluationFunctionH1(self, pawnToBeEvaluated, diceValue):
        # print "FINAL POSITION INDEX: ", pawnToBeEvaluated.getCumulativeDiceValue()+ diceValue
        finalPawnPosition = self.__currentPlayer.getPathArray()[(pawnToBeEvaluated.getCumulativeDiceValue()+ diceValue)%25]
        squareToCheck = self.__squaresDictionary[finalPawnPosition]
        #Distance from the goal
        ...
        if(squareToCheck.isSafeSquare() == True):
            evaluatedValue+=8
        print "Naive Level 1 evaluation for pawn", pawnToBeEvaluated.getName()[1],"of player: ", self.__currentPlayer.getPlayerName()," when the dice value is: ", diceValue,": ", str(evaluatedValue/float(distanceToGoalSquare))
    ...
    def getPawnIdOfPawnToMoveNaively(self, diceValue):
        ...
        #first level
        for pawn in movablePawnsList:
            evaluationValue = self.naiveEvaluationFunctionH1(pawn, self.__dice)
